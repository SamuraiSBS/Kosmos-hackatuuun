    ...
    <main className="analysis-scene scene-page">
      ...
      {phase === 'error' ? (
        ...
        <div className="analysis-results">
          ...
          <div className="layer-explanation"><div><span className="eyebrow">ИНТЕРАКТИВНЫЙ СЛОЙ</span><p>{showSignal ? 'Оранжевые ячейки — участок, который нужно проверить на месте.' : 'Нажми, чтобы увидеть, как ДЗЗ выделяет скопление мусора на снимке.'}</p><small>ДЗЗ — дистанционное зондирование Земли · {displayData.observation} · учебный снимок</small></div><button className="secondary-button" type="button" aria-pressed={showSignal} onClick={() => {
            if (showSignal) {
              setShowSignal(false)
              return
            }
            ...
            setShowSignal(true)
          }}>{showSignal ? 'Скрыть слой' : 'Показать слой'}</button></div>
